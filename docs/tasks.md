...
- **구현 진행**: `src/config.js` 완료 — TDD 27개 테스트 통과(`defaultConfig`/`resolveConfigPath`/`validateSelection`/`parseConfig`/`mergeSelection`/`load`/`save`). `bin/cli.js` 는 아직 미구현 플레이스홀더.
...
(없음 — `src/config.js` 완료. 다음: 예정 1번 `src/launch.js`)

## 예정 (구현 태스크)

### 1. 플래그 조립 + 실행 `src/launch.js`
